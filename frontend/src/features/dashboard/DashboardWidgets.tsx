...
import { Card, Badge, Typography, ErrorState, CardSkeleton } from '@/components/ui'
import { formatDate, getStatusColor } from '@/utils/format'
import type { DashboardIncident, DashboardStatusItem, HealthCheck, Incident } from '@/types/api'
...
export function EndpointStatusList({ items, isLoading, isError, onRetry }: EndpointStatusListProps) {
  if (isLoading) return <CardSkeleton />
  if (isError) return <Card><ErrorState onRetry={onRetry} /></Card>
          ...
          >
            <div>
              ...
              <Typography variant="caption" color="text">
                {item.monitoring_duration_days}d monitored
              </Typography>
            </div>
...
export function IncidentTimeline({
  ...
  return (
    <Card>
      ...
      {!incidents?.length ? (
        <Typography variant="body2" color="text">No incidents recorded</Typography>
      ) : (
        <div className="space-y-3">
          {incidents.map((inc) => {
            const name =
              'endpoint_name' in inc && inc.endpoint_name
                ? inc.endpoint_name
                : endpointNames[inc.endpoint_id] ?? `Endpoint #${inc.endpoint_id}`
            return (
              <div key={inc.id} className="flex items-start gap-3 border-l-2 border-error/50 pl-3">
                <div className="flex-1">
                  <Typography variant="button" color="white">{name}</Typography>
                  <Typography variant="caption" color="text" className="block">
                    {formatDate(inc.started_at)}
                    {inc.is_resolved && inc.resolved_at ? ` → ${formatDate(inc.resolved_at)}` : ' → Active'}
                  </Typography>
                </div>
...
export function HealthCheckList({ checks, isLoading, isError, onRetry, limit = 5 }: HealthCheckListProps) {
  if (isLoading) return <CardSkeleton />
  if (isError) return <Card><ErrorState onRetry={onRetry} /></Card>
  ...
  return (
    <Card>
      ...
      <div className="space-y-2">
        {display.map((check) => (
          <div key={check.id} className="flex items-center justify-between rounded-lg bg-white/5 px-3 py-2">
            <div>
              <Typography variant="button" color="white">
            ...
            <div className="flex items-center gap-2">
              <Typography variant="caption" color="text">{check.response_time}ms</Typography>
      ...
      <div className="space-y-2">
        {items.map((item, i) => (
          ...
          >
            ...
            <Typography variant="button" color="white" className="flex-1 truncate">
              {item.name}
            </Typography>
            <Typography variant="caption" color="text">{item.value}</Typography>
          </Link>
        ))}
      </div>
    </Card>
