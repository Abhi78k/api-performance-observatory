...
export function getStatusColor(status: string): 'success' | 'error' | 'warning' | 'info' {
  const s = status.toLowerCase()
  if (s === 'healthy' || s === 'resolved' || s === 'success') return 'success'
  if (s === 'unhealthy' || s === 'failed' || s === 'critical' || s === 'active') return 'error'
  if (s === 'degraded' || s === 'slow' || s === 'medium' || s === 'high') return 'warning'
  return 'info'
}
